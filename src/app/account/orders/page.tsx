...
// const API_URL = process.env.NEXT_PUBLIC_API_URL || "https://ecommerce-online-8r1c.onrender.com/api";
...
export default function OrdersPage() {
  ...
  // Toggle submenu "Tài khoản của tôi"
  ...
  const tabs = [
    { key: "ALL", label: "Tất cả" },
    { key: "PENDING", label: "Chờ xác nhận", match: (s: string) => s?.toLowerCase() === "pending" },
    { key: "PROCESSING", label: "Vận chuyển", match: (s: string) => s?.toLowerCase() === "processing" },
    { key: "COMPLETED", label: "Hoàn thành", match: (s: string) => s?.toLowerCase() === "completed" },
    { key: "CANCELLED", label: "Đã hủy", match: (s: string) => s?.toLowerCase() === "cancelled" },
    { key: "REFUND", label: "Trả hàng/Hoàn tiền" },
  ];
    ...
    <div className="max-w-6xl mx-auto p-6">
      <div className="grid grid-cols-12 gap-6">
        <aside className="col-span-3 bg-white rounded-lg border h-fit">
          ...
          <nav className="p-2 text-sm">
            <div className="px-4 py-3 cursor-pointer hover:text-orange-500">Thông Báo </div>
            <div className="mt-2">
              <div
                className="px-4 py-3"
              ...
              {openAccountMenu && (
                <div className="ml-10 mt-1 text-gray-700 space-y-2">
                  <a href="/account" className="block hover:text-orange-500">Hồ Sơ</a>
                  <a href="/" className="block hover:text-orange-500">Ngân Hàng</a>
                  <a href="/" className="block hover:text-orange-500">Địa Chỉ</a>
                  <a href="/account/edit" className="block hover:text-orange-500">Đổi Mật Khẩu</a>
                  <a href="/" className="block hover:text-orange-500">Cài Đặt Thông Báo</a>
                  <a href="/" className="block hover:text-orange-500">Những Thiết Lập Riêng Tư</a>
                  <a href="/account" className="block hover:text-orange-500">Thông Tin Cá Nhân</a>
                </div>
              )}
            </div>
            <div className="px-4 py-3 bg-orange-50 text-orange-600 font-medium rounded-r-full">Đơn Mua </div>
        ...
        <section className="col-span-9">
          ...
          {loading ? (
            ...
            <OrderList orders={filtered} setOrders={setOrders} />
          )}
        </section>
      </div>
    </div>
    ...
    <div className="mt-4 space-y-4">
      {orders.map((order: any) => (
        <div key={order.id} className="border rounded bg-white">
          ...
          <OrderBody order={order} setOrders={setOrders} />
        </div>
      ))}
    </div>
  );
...
/* ORDER HEADER */
function OrderHeader({ order }: any) {
  return (
    <div className="flex items-center justify-between px-4 py-3 border-b">
      <div className="text-gray-700">Mã đơn: #{order.id}</div>
...
function StatusBadge({ status }: any) {
  ...
  return (
    <span className={`px-3 py-1 rounded-full text-xs font-medium ${colors[s] || "bg-gray-100 text-gray-800"}`}>
      {status}
    </span>
  );
}
...
/* ORDER BODY */
function OrderBody({ order, setOrders }: any) {
  return (
    <div className="px-4 py-3">
      ...
      {Array.isArray(order.items) && order.items.length > 0 && (
        <div className="mt-3 border-t pt-3">
          <div className="space-y-3">
            {order.items.map((item: any) => (
              <OrderItem key={item.id} item={item} isCompleted={String(order.status).toLowerCase() === "completed"} />
            ))}
          </div>
        </div>
      )}
      ...
      {/* Hủy đơn */}
      {String(order.status).toLowerCase() === "pending" && (
        <CancelButton order={order} setOrders={setOrders} />
      )}
    </div>
  );
}
...
/* FORMAT ADDRESS */
function formatAddress(raw: any) {
  let addr = raw;
  if (typeof raw === "string") {
    try {
      addr = JSON.parse(raw);
  ...
  if (!addr) return "Địa chỉ: Không có dữ liệu";

  const line = addr.address || addr.address_line || "";
  const region = addr.region ? `, ${addr.region}` : "";
  const name = addr.full_name ? `${addr.full_name} — ` : "";
  const phone = addr.phone ? ` (${addr.phone})` : "";
  const type = addr.type === "office" ? " — Văn phòng" : addr.type === "home" ? " — Nhà riêng" : "";

  return `Địa chỉ: ${name}${line}${region}${phone}${type}`;
}

/* ITEM */
function OrderItem({ item, isCompleted }: any) {
  const firstImg =
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1">
        ...
        <p className="text-sm text-gray-500">
          Số lượng: {item.qty} × {Number(item.price).toLocaleString("vi-VN")} ₫
        </p>
...
/* HỦY ĐƠN */
function CancelButton({ order, setOrders }: any) {
  const token = localStorage.getItem("token");
  ...
  const cancelOrder = async () => {
    ...
    try {
      ...
      const reload = await fetch(`${API_URL}/orders/my`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <button
      onClick={cancelOrder}
      className="mt-3 px-4 py-2 rounded bg-red-50 text-red-600 border border-red-200 hover:bg-red-100"
    >
      Hủy đơn hàng
    </button>
  );
      ...
      {!show ? (
        ...
        <div className="mt-2 p-3 border rounded bg-gray-50">
          <p className="text-sm mb-2">
            Đánh giá cho: <span className="font-medium">{productName}</span>
          </p>
          ...
          <div className="mt-2 flex gap-2">
            <button onClick={submit} className="px-3 py-1 bg-green-600 text-white rounded text-sm">
              Gửi
            </button>
            <button onClick={() => setShow(false)} className="px-3 py-1 border rounded text-sm">
              Hủy
            </button>
          </div>
        </div>
